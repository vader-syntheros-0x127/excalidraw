...
// STRL: removed GitHub/X/Discord/YouTube icon imports with their command-palette links
...
// STRL: local-first scene init. Collaboration/backend import removed; we load
// from localStorage and still support opening a scene via a #url= link.
...
const ExcalidrawWrapper = () => {
  ...
  const loadImages = useCallback(
    (data: ResolutionType<typeof initializeScene>, isInitialLoad = false) => {
      ...
      // STRL: local-only image loading (collab/firebase file fetch removed).
      const fileIds =
    ...
    <div style={{ height: "100%" }} className="excalidraw-app">
      <Excalidraw
        ...
        UIOptions={{
          canvasActions: {
            toggleTheme: true,
            // STRL: drop the backend-dependent Excalidraw+ cloud export and
            // shareable-link export; keep local file export only.
            export: {
              saveFileToDisk: true,
            },
          },
        ...
        autoFocus={true}
        theme={editorTheme}
        // STRL: AI off until the user configures a BYO endpoint (Menu → AI settings).
        aiEnabled={isAiConfigured()}
        renderTopRightUI={(isMobile) => {
          if (isMobile || !excalidrawAPI) {
            return null;
          }
          // STRL: local-first build — only our export button (no collab UI).
          return (
            <div className="excalidraw-ui-top-right">
              <StrlExportButton excalidrawAPI={excalidrawAPI} />
            </div>
          );
        }}
      ...
      >
        ...
        <CommandPalette
          customCommandPaletteItems={[
            // STRL: removed GitHub / X / Discord / YouTube community links (rebrand)
            {
