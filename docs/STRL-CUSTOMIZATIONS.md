...
**(c) Save-target confinement — `strl:opened` allowlist (post-Tier-B security reassessment, 2026-06-01).** An adversarial review of the IPC surface found the read-hardening of (b) had a **write-side twin (CRITICAL)**: `strl:opened` adopted _any_ renderer-supplied path as `activeFilePath`, which `strl:save-file` then overwrites **in place, no dialog** on the next save. A compromised renderer (e.g. via a malicious `.excalidraw` exploiting a renderer/dep bug) could `confirmOpened('~/.bashrc')` then in-place-save attacker bytes → silent arbitrary-file write = code execution — a full sandbox escape. **Fix** (`main.ts`): `openFilePath` records every path main itself issues for opening in a `sceneOpenPaths` set, and `strl:opened` adopts a path _only if it is in that set_ (mirroring the `strl:open-recent` allowlist). This also closes a **medium read-twin** (the renderer could otherwise point the file-watcher at `~/.ssh/*` and receive the bytes via `strl:external-change`, whose `handleExternalChange` has no extension guard) because the watch target now only ever derives from a validated open. **Proven by a CDP regression test** (`cdp-runtime-smoke.mjs` check #4 — the live `confirmOpened(sentinel)` + in-place-save exploit leaves the sentinel file intact; harness now 7/7). The same pass also: tightened `isValidOrigin` to reject wildcard hosts (`https://*.evil.com`) before they broaden the CSP `connect-src`/`img-src`; and added an **http(s)-only guard at the AI fetch sites** (`byoStreamFetch`/`byoImageGen`) so a saved `file:`/`data:`/`javascript:` endpoint is never fetched (the web build has no CSP backstop). **Residuals (accepted, low / desktop-secondary):** ~~the web build ships no CSP~~ — **resolved**, the web build now ships a `<meta>` CSP + an authoritative `/public/_headers` CSP and self-hosts all fonts (§13.9 residual-clearing pass); the BYO API key is plaintext in `localStorage` (verified _never_ serialized into scenes/exports/logs); mermaid XSS-safety relies on the pinned `mermaid@11.15.0` `securityLevel:'strict'` default (re-verify on any mermaid bump). A **release-artifact audit of the packaged v0.2.0 desktop build** (2026-06-02 — verdict **GO**, 4-dimension read-only audit of the extracted `app.asar`) confirmed no network phone-home, no embedded secrets, fonts fully self-hosted, and the Electron hardening (strict `app://` CSP, `contextIsolation`/`sandbox`/`nodeIntegration:false`, the `sceneOpenPaths` save-target allowlist, `isValidOrigin` wildcard rejection) **byte-faithful in the shipped `main.js`**. One inert residual: a `cdnjs.cloudflare.com/pdfobject.min.js` string vendored inside `jsPDF` — **dead code** (our PDF export uses `.output('arraybuffer'/'bloburl')`, never `pdfobjectnewwindow`) and CSP-blocked regardless; safe to ignore (or strip on a future jsPDF bump).
...
This took the tree from **63 → 0** Snyk findings (commit `6cabed28`).

**Then upgraded vite 6 → 8 (latest) to stay current.** vite 8 needs a larger cascade: `vite` → **8.0.14**, `@vitejs/plugin-react` 4.7.0 → **5.2.0** (4.x caps at vite 7), `vite-plugin-pwa` 0.21.1 → **1.3.0** (0.21 caps at vite 6), and a vitest bump: `vitest` 3.0.6 → **4.1.7** + `@vitest/coverage-v8`/`@vitest/ui` → **4.1.7** + `vitest-canvas-mock` 0.3.3 → **1.1.4** (vitest 3.0.6's own vite dep is `^5‖^6`, so vite 8 forces a vitest major). `esbuild` initially stayed pinned at 0.25.12 (vite 8 _tolerates_ it as an out-of-range **optional** peer); later bumped to **0.28.0** to move into range — see "esbuild override → 0.28.0" below. Two `tsconfig.json` fixes were required:

- **`moduleResolution: "node"` → `"bundler"`** — vite 7+ ship types behind `exports`, which classic node resolution can't read (`vite.config.mts: Cannot find module 'vite'`). `bundler` is Vite's own recommendation; verified it doesn't ripple into app source (typecheck stays 0).
- **added `"**/build"`to`exclude`** — with `allowJs: true`, `vite-plugin-checker`'s tsc was pulling in the just-emitted `excalidraw-app/build/assets/\*.js` chunks mid-render (`TS6053: File not found` as hashed names changed). Build output should never be type-checked; vite 8's pipeline timing exposed it.

**Cost of "latest":** 0 security vulns, but vite 8 pulls in `lightningcss@1.32.0` (+ 11 platform binaries) flagged **MPL-2.0 license** — **12 license findings** (build-tool, not shipped in the app bundle, no CVEs). These are left **visible** (not suppressed) by choice; vite 6.4.2 had 0 findings, so the latest is currency, not security. Re-verified under vite 8: typecheck 0, renderer + AppImage builds, boot smoke, CDP 6/6, `vitest@4.1.7` math suite 29/29, all 5 node smokes. (vitest 4 emits a deprecation _warning_ for a pre-existing nested `vi.mock` in `tests/helpers/mocks.ts` — not an error.) `build:packages` (library-publish path, unused by the app) stays pre-existing-broken on its `yarn gen:types` step (yarn→pnpm leftover, unrelated).

**esbuild override 0.25.12 → 0.28.0 (peer-range alignment).** vite 8.0.14 declares `peerDependencies.esbuild: "^0.27.0 || ^0.28.0"` with `peerDependenciesMeta.esbuild.optional: true` — an _optional_ peer (vite 8 bundles via **rolldown**; esbuild is only a compat/transform path), so 0.25.12 built fine but sat _below_ the declared range. The single-version override is **load-bearing, not a downgrade**: nothing in the tree _installs_ 0.27/0.28 on its own — vite's esbuild is an un-installed optional peer, and the packages that actually pull esbuild (`esbuild-sass-plugin@^0.19.4`, the webpack/size-limit path) ask for old **0.19.x**; under `node-linker=hoisted` two majors collide → the EPIPE. So the override forces a _single_ esbuild _up_ into vite 8's window. Bumped the override to **0.28.0** (latest; published 2026-04-02, clears `minimum-release-age=1440`). Applied via a **clean reinstall** (`pnpm rm:node_modules` → hardened `pnpm install` under Safe Chain) so no stale 0.25.12 binary lingers — verified one `esbuild@0.28.0` in the tree, JS wrapper == binary == 0.28.0, transform canary OK. **Supply-chain:** esbuild 0.28.0 has **0 Snyk findings** and no transitive npm deps → zero impact on the shipped surface. (A _dev-inclusive_ SCA surfaces ~20 **pre-existing dev-only** toolchain vulns — `@size-limit/preset-big-lib` [tar-fs/ws/webpack/serialize-javascript], `rewire`/eslint 7 [flatted], electron-builder [inflight], jsdom [follow-redirects/js-yaml] — unrelated to esbuild and not in the app/desktop runtime; flagged for a future dep-currency pass.) Re-verified: typecheck 0, app + desktop renderer builds (✓ ~2 s, no host/binary mismatch), 4 node smokes, `vitest@4.1.7` 29/29, AppImage boot smoke (`hasEditor:true,dark:true`), CDP 6/6.

**Tier B dependency currency (follow-on to the esbuild/Tier A work).** After a per-package breaking-change research pass, bumped: root devDeps `jsdom` 22.1.0→**29.1.1**, `@vitejs/plugin-react` 5.2.0→**6.0.2** (v6 drops bundled Babel; JSX/Fast-Refresh via Oxc on vite 8 — our bare `react()` call is a drop-in), `vite-plugin-checker` 0.7.2→**0.11.0**, `dotenv` 16.0.1→**17.4.2**; excalidraw-app `react`/`react-dom` 19.0.0→**19.2.6**, `jotai` 2.11.0→**2.20.0**, `idb-keyval` 6.0.3→**6.2.4**. Because upstream `packages/excalidraw` hard-pins `jotai` and other consumers pull `react`, the app-manifest bump alone left **split versions** (dual React = invalid-hook hazard); collapsed to one each via root `pnpm.overrides` (`react`/`react-dom`=19.2.6, `jotai`=2.20.0 — 19.2.6 satisfies upstream's `^19` peer).

- **Transitive-vuln overrides (added):** `tar-fs@2`=2.1.4, `ws@8`=8.21.0, `serialize-javascript`=7.0.5, `webpack`=5.107.2, `yaml@1`=1.10.3, `js-yaml@4`=4.1.1, `js-yaml@3`=3.14.2, `flatted`=3.4.2 (critical PP CVE-2026-33228), `follow-redirects`=1.16.0, `@tootallnate/once@2`=2.0.1 (inert — jsdom 29 dropped it). Clears the dev-tree findings from the size-limit/webpack, eslint-cache, http-server, and jsdom paths.
- **Snyk SCA (dev-inclusive): 20 → 5** = **2 accepted vulns** + 3 MPL license (axe-core, lightningcss×2, unshipped). The 2 accepted are dev-only with **no in-scope fix**: `eslint@7.32.0` (recursion DoS; fix is eslint 9.26.0) and `inflight@1.0.6` (archived 2024, no fixed version). Bonus: react→19.2.6 also clears the 19.0–19.2.3 RSC advisory (fixed 19.2.4; not applicable to this client-only fork anyway).
- **Held → Tier C:** `rewire`→9 and `vite-plugin-checker`→0.14.1 both **cascade into an ESLint-9 + flat-config migration** (rewire@6 is the sole hard installer of eslint@7.32.0; checker ≥0.12 peers eslint ≥9.39). That one migration also retires the eslint DoS **and** inflight findings (both ride the same eslint-7 chain). Left for a dedicated workstream with the rest of Tier C (prettier 2→3 = whole-repo reformat, typescript 5.9→6, chai 4→6, husky/lint-staged, i18next-browser-languagedetector 6→8).
- **Verification:** typecheck 0; app + desktop renderer builds; 4 node smokes; **full vitest baseline-compared** — pre-Tier-B HEAD already fails 11 (jest-not-defined in collision.test, library/search/history — pre-existing upstream-test debt in the fork), Tier B added exactly **4 benign snapshot drifts** (react 19.2 `useId` `:rN:`→`_r_N_` + jsdom 29 CSS-serialization), updated in 3 `.snap` files (adversarially diff-checked: balanced ±, no content change); AppImage boot + CDP 6/6 on the react-19.2 renderer; a 3-agent adversarial review returned clean on manifest/lockfile, snapshots, and findings-retired. `test:code`/eslint is impractically slow on the full monorepo (type-aware lint; CI disabled) and is unaffected by Tier B (no source changed; eslint runtime verified intact on individual files).

**Tier C — ESLint 9 flat-config migration.** Decided to OWN the lint toolchain rather than wait for upstream (checked `upstream/master`: 0 commits ahead, still on eslintrc + `eslint-config-react-app@7` + `prettier@2`, no flat config). Replaced the entire eslintrc stack with a single **`eslint.config.mjs`** (flat config, eslint **9.39.4**): deleted `.eslintrc.json` (root + element + common) + `packages/eslintrc.base.json`; dropped `@excalidraw/eslint-config` + `eslint-config-react-app@7` (both eslintrc-only, tied to `@typescript-eslint` v5 which cannot run on eslint 9) and `eslint-plugin-prettier` (its v5 needs prettier 3 — instead keep `eslint-config-prettier` to disable conflicting rules and let the separate `prettier`/`test:other` tooling check formatting). Added: `eslint` 9.39.4, `@eslint/js`, `typescript-eslint` 8.60.0, `eslint-plugin-react` 7.37.5 (+ a root override — upstream pins 7.32.2 which lacks eslint-9 support), `-react-hooks` 7.1.1, `-jsx-a11y` 6.10.2, `globals`; bumped `vite-plugin-checker`→**0.14.1** (peers eslint ≥9.39.4). Also removed the now-redundant **`rewire`** devDep (the sole hard installer of vulnerable eslint@7.32.0) + its dead `scripts/build-node.js` (required the absent `react-scripts`). `.eslintignore`→`.prettierignore`; `test:code` drops `--ext` (removed in eslint 9). **Result: `eslint@7.32.0` (CVE-2025-50537 recursion DoS) is CLEARED** (Snyk 5→4); the one remaining accepted vuln, `inflight`, is **not** an eslint path — it rides electron-builder's `@electron/asar`→glob (a separate `@electron/asar@4.1.0` bump, deferred to avoid a packaging-risk re-verify). **The config is behavior-preserving, not "modern-recommended":** it replicates the prior @excalidraw + react-app rule set (NOT `tseslint.configs.recommended`, which would flag thousands of upstream lines) and relaxes `js.configs.recommended` to match react-app (`no-loss-of-precision`/`no-redeclare`/`no-case-declarations` off, `no-empty` allowEmptyCatch, etc.). **Lint is scoped to the code we own** (excalidraw-app, packages/strl-\*, desktop, scripts) and **ignores the upstream `packages/excalidraw|element|common|math|utils|fractional-indexing`** (upstream's to lint; they carry their own inline disable directives). The upstream "no literal JSX text → use `t()`" i18n rule is OFF (STRL UI text is intentionally literal). **`test:code` is now `--max-warnings=0` GREEN and fast (~8 s, was >12 min — no type-aware rules).** Verified: typecheck 0, 4 node smokes, app + desktop builds, vitest math 29/29, AppImage boot + **CDP 7/7**, `test:other` green. The rest of Tier C (prettier 2→3, typescript 5.9→6, chai 4→6, husky/lint-staged) stays **held** — whole-repo/upstream-divergent for low value.

**Build-tooling modernization (post-Tier-C).** Exhaustively analyzed the last accepted vuln, `inflight@1.0.6` (3-agent workflow): it rides `glob@7.2.3`, which had **four** dev-tool parents, and **cannot** be cleared by overriding glob/inflight (no glob version is both inflight-free _and_ glob@7-callback-API-compatible) — Snyk's own `@electron/asar@4.1.0` remediation wouldn't even clear it (3 other paths remain) and is a risky major. The right fix is removing/modernizing the **stale tools**, not patching transitives: **(1)** removed the dead **`@size-limit/preset-big-lib@9.0.0`** from `packages/excalidraw/package.json` + deleted `.github/workflows/size-limit.yml` — an **unused** (no `size` script; we don't publish the library) 3-majors-stale bundle-size tool that dragged in **`puppeteer-core@13.7.0`** (12 majors behind latest 25.1.0 — a ~2022 headless Chromium), `estimo`, `@size-limit/webpack`, and `rimraf@3`→glob@7. **(2)** `workbox-build`/`workbox-window` → **7.4.1** via root overrides (glob@^11, no inflight) — also fixes the standing vite-plugin-pwa unmet-peer warning. **(3)** `electron-builder` **26.8.1→26.11.1** (desktop devDep — currency on the installer tool; 26.12.0 was too fresh for `minimum-release-age`). **Result:** removed the ancient Chromium + dead toolchain and fixed the peer warning; **`inflight` dropped from 4 paths to 2** (only electron-builder's `@electron/asar@3.x` + `electron-winstaller`→`rimraf@2`, both internal). `inflight` therefore **remains an accepted dev-only finding** (Snyk steady at 1 vuln + 3 license) — it will clear when electron-builder itself adopts `@electron/asar@4` (which raised the Node floor to ≥22.12); forcing it now provably breaks asar packaging. Verified through the full gate: Snyk SCA/Code before build (no new findings), typecheck 0, 4 node smokes, vitest sanity, app build (`sw.js` still generated by workbox 7.4.1), AppImage on **electron-builder 26.11.1** + boot smoke + **CDP 7/7**, `test:code`/`test:other` green.

**Residual-clearing pass — SAST→0, inflight→green, web CSP + web font self-hosting (2026-06-02).** A targeted pass to clear the remaining accepted residuals rather than carry them:

- **`inflight` → Snyk-green (the previously-deferred asar bump, now done).** Added `pnpm.overrides` `@electron/asar: 4.2.0` (4.x uses `glob@^13`, no inflight; Node floor ≥22.12 is met — we run 22.22.2). This removes the **mac-universal** path (`@electron/asar@3.4.1`→glob@7), dropping `inflight` from **2 paths to 1**. The sole remainder is the **Windows-Squirrel** chain `electron-builder-squirrel-windows`→`electron-winstaller`→`temp@0.9.4`→`rimraf@2`→glob@7 — unfixable by override (`temp` is at its latest and hard-deps `rimraf@2`; `rimraf@3` still pulls glob@7, `rimraf@4` drops the callback API `temp` needs) and never executed on our Linux/macOS pipeline. It is now a **documented, time-boxed `.snyk` ignore** (`SNYK-JS-INFLIGHT-6095116`, expires 2026-12-02) with the full rationale — so the dashboard is green without silent suppression. **The asar@4 override is fully packaging-verified** (AppImage builds, `app.asar` packs, boot smoke `hasEditor:true`, **CDP 7/7**) — disproving the earlier "forcing it breaks asar packaging" worry now that electron-builder is 26.11 + Node 22.
